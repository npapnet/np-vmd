...
tmck.set_iv(x0s = np.array([[0, 0]]).T, 
            dx0s = np.array([[0,0]]).T)
tmck.set_excitation(
    B = None,
    # B=np.array([[0,0],[0,1]]),
...
r_mdof = MdofForcedResponseSingleExcitation(mdof_sys = tmck,
             node=0, Fmag=F1_N, w_exc_radps=w_Exc_radps, phi_exc_rad=0)
...
for i in range(2):
    ...
    print(f"  Response:")
    print(f"           Xss: {lst_pc_response_params[i].get('Xss'):.3g} phi_ss: {lst_pc_response_params[i].get('phi_ss'):.3g}")
    print(f"           Xtr: {lst_pc_response_params[i].get('X_tra'):.3g} phi_tr: {lst_pc_response_params[i].get('phi_tra'):.3g}")
#%%
