...
class TDOF_modal():
    ...
    def __perform_initial_setup(self):
        ...
        self.ls, self.vs = np.linalg.eig(self.Ktilde) # eigenvalues and eigenvectors fro the Ktilde matrix
...
class MdofForcedResponseSingleExcitation():
    ...
    def __init__(self, mdof_sys: TDOF_modal, node: int, Fmag: float, w_exc_radps: float, phi_exc_rad: float=0):
        """
    ...
    @property
    def dofs(self)->int:
        return self.mdof_sys.dofs

    def __create_fparams(self, ):
        ...
        self.f_params[self._excitation_node, :] = np.array([self._force_mag_N, self._w_exc_radps, self._phi_exc_rad])
    ...
    def calc_response(self):
        ...
        for k in range(self.dofs):
            _modal_sys = SDOF_system.from_wn_mz(wn=self.mdof_sys.wns[k], m=1, zeta=self.mdof_sys.zs[k])
            # the rotated initial conditions are placed here
            _modal_resp_parms = _modal_sys.response_params(
                x0=self.r0s[k,0], v0=self.dr0s[k,0], 
                F0=self.Fmag_pc[k], w=self._w_exc_radps)
        ...
        self.xs_ABmag_orig = self.mdof_sys.Smat.dot(self.x_mags_pc).dot(self.angle_2dArray)
    ...
    def jth_response_func(self, j: int, update: bool=False) -> str:
        ...
        Xss, phi_ss = convert_harmonic_to_cos(self.xs_ABmag_orig[j,0], self.xs_ABmag_orig[j,1])
        X_tra = res.get("X_tra")
        ...
        response_func = lambda t: X_tra*np.cos(self.mdof_sys.wns[j]*t+phi_tra) + Xss*np.cos(self._w_exc_radps*t+phi_ss)
